...
[ "$GITHUB_PAGES_USER" ] || {
  echo "ERROR: Environment variable GITHUB_PAGES_USER is required"
  exit 1
}
[ "$GITHUB_PAGES_REPO" ] || {
...
echo "GITHUB_PAGES_USER=$GITHUB_PAGES_USER"
...
helm version -c
helm init -c
helm repo add self https://$GITHUB_PAGES_USER.github.io/$GITHUB_PAGES_REPO

echo ">> Checking out $GITHUB_PAGES_BRANCH branch from $GITHUB_PAGES_USER/$GITHUB_PAGES_REPO"
cd /tmp/helm/publish
mkdir -p "$HOME/.ssh"
ssh-keyscan -H github.com >> "$HOME/.ssh/known_hosts"
git clone -b "$GITHUB_PAGES_BRANCH" "git@github.com:$GITHUB_PAGES_USER/$GITHUB_PAGES_REPO.git" .
...
echo ">> Publishing to $GITHUB_PAGES_BRANCH branch of $GITHUB_PAGES_USER/$GITHUB_PAGES_REPO"
